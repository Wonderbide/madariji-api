-- Update Word Analysis prompt to capture proper Arabic linguistic data
-- Version 4.0: Adds jidar, masdar, wazn for nouns; root now triliteral with spaces

-- First, deactivate the old version
UPDATE prompt_library
SET is_active = false
WHERE prompt_key = 'WORD_ANALYSIS_V1'
AND is_active = true;

-- Insert the new version with proper linguistic fields
...
4.  **jidar** (الجذر المجرد) : La forme nue du mot, dévêtue de toutes les particules annexées :
...
5.  **root** (الجذر الثلاثي) : La racine trilitère ou quadrilitère avec espaces entre les lettres.
...
6.  **masdar** (المصدر) : Le nom d''action associé au mot :
...
7.  **wazn** (الوزن) : Le schème/pattern morphologique :
...
8.  Remplis l''objet `details` selon le type grammatical du "Mot Cible" :
...
9.  **TRADUCTION DYNAMIQUE** : Fournis la traduction du "Mot Cible" dans la langue spécifiée (%s). TOUJOURS donner la traduction du mot seul, comme dans un dictionnaire.

10. **DISTINCTION ABSOLUE entre "translation" et "meaning"** :
...
11. **Identifie les champs lexicaux** : Fournis de 1 à 3 champs lexicaux dans `"lexical_fields"`.
